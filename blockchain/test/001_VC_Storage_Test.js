...
describe("VCStorageContract", function () {
  ...
  async function deployAsUser() {
    ...
    console.log("DRUGI nalog");
    console.log(otherAccount);
    const Lock = await ethers.getContractFactory("VCStorageContract");
  ...
  async function deployAsContract() {
    ...
    const [owner, otherAccount] = await ethers.getSigners();
    const Lock = await ethers.getContractFactory("VCStorageContract");
    const lock = await Lock.deploy(otherAccount.address);
    return { lock, owner, otherAccount };
  }

  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      const { lock, owner } = await loadFixture(deployAsUser);

      expect(await lock.owner()).to.equal(owner.address);
    });

    it("Should set the owner from costructor", async function () {
      const { lock, owner, otherAccount } = await loadFixture(deployAsContract);
